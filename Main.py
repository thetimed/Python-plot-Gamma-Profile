...
drawdown_strike = 0.9 * spotPrice
breakup_strike = 1.1 * spotPrice
...
#So we assume that to calculate the gamma -

#Let us put out a simple gamma model, 

#Gamma Exposure = Gamma * open Interest * Contract Size * Spot Price

df['Call Gamma Exposure'] = df['CallGamma'] * 100 * spotPrice
df['Put Gamma Exposure'] = df['PutGamma'] * 100 * spotPrice
...
#To limit it down to up and down 20%

plt.xlim(drawdown_strike,breakup_strike) 
plt.show()
...
levels = np.linspace(drawdown_strike, breakup_strike, 10)
